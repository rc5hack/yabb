            ...
            if (!$INFO{'clean'}) {
                  ...
                  foreach my $testboard (@allboards) {
                        ...
                        if (-e "$convboardsdir/$testboard.ttl") {
                              fopen(BOARDTTL, "$convboardsdir/$testboard.ttl") || &setup_fatal_error("Can not open $convboardsdir/$testboard.ttl", 1);
                              my $line = <BOARDTTL>;
                              fclose(BOARDTTL);
                              chomp $line;
...
sub PrepareConv {
      fopen(FILE, ">$boardsdir/dummy.testfile") || &setup_fatal_error("The CHMOD of the $boardsdir is not set correctly! Cannot write this directory!", 1);
      print FILE "dummy testfile\n";
      fclose(FILE);
      opendir(BDIR, $boardsdir) || &setup_fatal_error("The CHMOD of the $boardsdir is not set correctly! Cannot read this directory! ", 1);
      @boardlist = readdir(BDIR);
      closedir(BDIR);

      fopen(FILE, ">$memberdir/dummy.testfile") || &setup_fatal_error("The CHMOD of the $memberdir is not set correctly! Cannot write this directory!", 1);
      print FILE "dummy testfile\n";
      fclose(FILE);
      opendir(MBDIR, $memberdir) || &setup_fatal_error("The CHMOD of the $memberdir is not set correctly! Cannot read this directory! ", 1);
      @memblist = readdir(MBDIR);
      closedir(MBDIR);

      fopen(FILE, ">$datadir/dummy.testfile") || &setup_fatal_error("The CHMOD of the $datadir is not set correctly! Cannot write this directory!", 1);
      print FILE "dummy testfile\n";
      fclose(FILE);
      opendir(MSDIR, $datadir) || &setup_fatal_error("The CHMOD of the $datadir is not set correctly! Cannot read this directory! ", 1);
...
<form action="$set_cgi?action=setinstall" method="post">~;
      ...
      <tr valign="middle">
            <td width="100%" class="windowbg2" align="center">
                  <br />
                  <font color="red" size="+2">Sorry, you cannot continue until you have installed at least the "Digest::MD5" module.</font><br />
                  <br />
            </td>
      </tr>
      </table>~;
 ...
 <div class="bordercolor" style="padding: 0px; width: 100%; margin-left: 0px; margin-right: 0px;">
   <table width="100%" cellspacing="1" cellpadding="4">
      ...
      <br />
            <div style="float: left; font-family: verdana; width: 45%; text-align: left; font-size: 12px; padding-top: 2px; padding-bottom: 2px;">
            Forum Time Zone<br />
            <span style="font-size:small">If the time displayed here differs from your local time, then adjust it by changing the settings.</span>
            </div>
            <div style="float: left; font-family: verdana; width: 55%; text-align: left; font-size: 12px; padding-top: 2px; padding-bottom: 2px;">
...
sub SetInstall2 {
      if ($action eq "checkmodules" || $action eq "setinstall2") {
            ...
            $mbname =~ s/\"/\'/g; #'" make my syntax checker happy;
            ...
            $max_log_days_old = 90;
            $fadertime = 1000;
            $defaultusertxt = 'I Love YaBB 2.5.2!';
            $timeout = 5;
            $HotTopic = 10;
            $VeryHotTopic = 25;
            $barmaxdepend = 0;
...
sub SetupTemplate {
	...
	if    ($hour >= 12 && $hour < 18) { $wmessage = $maintxt{'247a'}; } # Afternoon
      ...
      if ($yycopyin == 0) {
            $output = q~<h1 style="text-align:center"><b>Sorry, the copyright tag <yabb copyright> must be in the template.<br />Please notify this forum's administrator that this site is using an ILLEGAL copy of YaBB!</b></h1>~; #';
      }
      if (fileno GZIP) {
            $| = 1;
            print GZIP $output;
